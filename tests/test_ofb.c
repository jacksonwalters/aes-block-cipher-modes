...
int main(void) {
    ...
    // Encrypt
    aes_ofb_encrypt(plaintext, ciphertext, sizeof(plaintext), iv, &ctx);

    if (memcmp(ciphertext, expected_ciphertext, sizeof(ciphertext)) != 0) {
        fprintf(stderr, "OFB encryption FAILED\n");
        return 1;
    }

    // Decrypt (same operation)
    aes_ofb_decrypt(ciphertext, decrypted, sizeof(ciphertext), iv, &ctx);

    if (memcmp(decrypted, plaintext, sizeof(decrypted)) != 0) {
        fprintf(stderr, "OFB decryption FAILED\n");
        return 2;
    }

    printf("OFB round-trip OK — plaintext recovered correctly.\n");
